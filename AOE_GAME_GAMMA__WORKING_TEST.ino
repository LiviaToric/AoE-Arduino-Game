...
//PILLARS #2 VARIABLES
int16_t xPillar2 = TFT_WIDTH;
//int16_t yPillar2 = TFT_HEIGHT - random(50, 60);

bool gameOver = false;
int16_t tally = 0;

//Array of Strings
char* words[7] = {"1007", "MULTI", "UW", "AOE", "CHEM", "JAVA"};
int16_t randomNumber = random(6);

//next pillar
int16_t next_xPillar = 100;
bool next_Pillar = false;
int16_t randomNumber2 = random(6);
...
void drawPillar()
{
  ...
  if (xPillar < TFT_WIDTH - 25) {
    tft.setTextColor(BLACK);
    tft.println(words[randomNumber]);
    tft.setTextColor(WHITE);
  }
  tft.fillRect(xPillar + movingRate + PILLAR_WIDTH, yPillar, movingRate, TFT_HEIGHT - yPillar, BACKGROUND_COLOR);
}

void drawPillar2(bool state)
{
  if(state){
    tft.fillRect(xPillar2, yPillar, PILLAR_WIDTH, TFT_HEIGHT - yPillar, PILLAR_COLOR);
    tft.setCursor(xPillar2, yPillar + 30);
    if (xPillar2 < TFT_WIDTH - 25) {
      tft.setTextColor(BLACK);
      tft.println(words[randomNumber2]);
      tft.setTextColor(WHITE);
    }
    tft.fillRect(xPillar2 + movingRate + PILLAR_WIDTH, yPillar, movingRate, TFT_HEIGHT - yPillar, BACKGROUND_COLOR);
  }
}
...
void loop(void)
{
  ...
  if (!gameOver)
  {
    ...
    //if student is at max height
    if (yStudent <= 124)
      fallingRate = 1;
    else if (yStudent >= TFT_HEIGHT - STUDENT_HEIGHT)
    {
      fallingRate = 0;
      yStudent = TFT_HEIGHT - STUDENT_HEIGHT;
    }
    else
      fallingRate += 0.2;
    ...
    if (next_Pillar && (xPillar2 <= 50 + STUDENT_WIDTH && xPillar2 >= 50 - PILLAR_WIDTH))
    {
      tally += 1;
      if (yStudent + STUDENT_HEIGHT > yPillar) {
        gameOver = true;
    ...
    drawPillar();
    if(xPillar<TFT_WIDTH/2 || next_Pillar){next_Pillar = true;}
    drawPillar2(next_Pillar); //summoning next pillar
    ...
    if(next_Pillar){
      xPillar2 -= movingRate;
    //if the pillar is out of the screen,
      if (xPillar2 < -PILLAR_WIDTH - 4) {
        xPillar2 = TFT_WIDTH;
        randomNumber2 = random(6);
      }
      if (xPillar2==0){next_Pillar = false;}
    }
